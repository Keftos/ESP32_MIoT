...
class BlinkerProtocol
{
    ...
    // #else
        ...
        void print(const String & key, const String & data);
            ...
            char * deviceName() { if (isInit) return conn->deviceName(); else return emptyStrings; }
            char * authKey()    { if (isInit) return conn->authKey(); else return emptyStrings;  }
            char * token()    { if (isInit) return conn->token(); else return emptyStrings;  }
            int init()          { return isInit ? conn->init() : false; }
    ...
    // #else
        ...
        char* dataParse()       { if (canParse) return conn->lastRead(); else return emptyStrings; }
